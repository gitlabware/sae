...
class PagosController extends AppController
{
    ...
    public function gen_excel_formato()
    {
		$id_edificio = $this->Session->read('Auth.User.edificio_id');
		$ambientes = $this->Ambiente->find('all',array(
			...
			'conditions' => array('Piso.edificio_id' => $id_edificio,'Ambiente.estado' => 'Activo','ISNULL(Ambiente.deleted)'),
			'order' => array('Piso.orden ASC','Ambiente.id ASC')
		));

        $nombre_excel = "formato-excel-pagos.xlsx";
...
//'color' => array('rgb' => 'FF7E00')
        ...
        $prueba = new PHPExcel();
        $prueba->getActiveSheet()->getColumnDimension('A')->setWidth(20);
        $prueba->getActiveSheet()->getColumnDimension('B')->setWidth(20);

		$prueba->getActiveSheet()->mergeCellsByColumnAndRow(0, 2, 14, 2);
		$prueba->getActiveSheet()->mergeCellsByColumnAndRow(0, 3, 14, 3);
		
        $style1 = array('alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER, 'vertical' =>
            PHPExcel_Style_Alignment::VERTICAL_CENTER), 'font' => array('size' => 15, 'bold' => true));
        $prueba->getActiveSheet()->getStyle("A2:O3")->applyFromArray($style1);
        $prueba->setActiveSheetIndex(0)->setCellValue("A2", "ESTADO GENERAL DE CUENTAS POR COBRAR");
		$prueba->setActiveSheetIndex(0)->setCellValue("A3", "GESTION ----");
		$prueba->getActiveSheet()->getStyle("A5:O5")->applyFromArray($borders2);
        $prueba->setActiveSheetIndex(0)->setCellValue("A5", "PISO");
        $prueba->setActiveSheetIndex(0)->setCellValue("B5", "AMBIENTE");
        $prueba->setActiveSheetIndex(0)->setCellValue("C5", "ENE");
        $prueba->setActiveSheetIndex(0)->setCellValue("D5", "FEB");
        $prueba->setActiveSheetIndex(0)->setCellValue("E5", "MAR");
        $prueba->setActiveSheetIndex(0)->setCellValue("F5", "ABR");
        $prueba->setActiveSheetIndex(0)->setCellValue("G5", "MAY");
        $prueba->setActiveSheetIndex(0)->setCellValue("H5", "JUN");
        $prueba->setActiveSheetIndex(0)->setCellValue("I5", "JUL");
        $prueba->setActiveSheetIndex(0)->setCellValue("J5", "AGO");
        $prueba->setActiveSheetIndex(0)->setCellValue("K5", "SEP");
        $prueba->setActiveSheetIndex(0)->setCellValue("L5", "OCT");
        $prueba->setActiveSheetIndex(0)->setCellValue("M5", "NOV");
        $prueba->setActiveSheetIndex(0)->setCellValue("N5", "DIC");
        $prueba->setActiveSheetIndex(0)->setCellValue("O5", "TOTAL");

		$cont = 5;
		
		foreach($ambientes as $ambiente){
			$cont++;
			$prueba->getActiveSheet()->getStyle("A$cont:O$cont")->applyFromArray($borders2);
			$prueba->setActiveSheetIndex(0)->setCellValue("A" . $cont, $ambiente['Piso']['nombre']);
			$prueba->setActiveSheetIndex(0)->setCellValue("B" . $cont, $ambiente['Ambiente']['nombre']);
		}

        // foreach ($almacenes as $sucursal) {
        //     $nume++;
        //     $prueba->setActiveSheetIndex(0)->setCellValue(chr($nume) . "2", "$sucursal");
        // }
        // $prueba->setActiveSheetIndex(0)->setCellValue(chr($nume + 1) . "2", "TOTAL");

        // $prueba->getActiveSheet()->getStyle('A2:' . chr($nume + 1) . '2')->applyFromArray($borders);

        // $prueba->getActiveSheet()->setTitle("REPORTE DE SALDOS");
        ...
        //     $cont++;
        //     $total_pro = 0;
        //     $prueba->getActiveSheet()->getStyle("A$cont:" . chr($nume + 1) . "$cont")->applyFromArray($borders2);
        //     $prueba->setActiveSheetIndex(0)->setCellValue("A" . $cont, $producto['Producto']['tipo_producto']);
        //     $prueba->setActiveSheetIndex(0)->setCellValue("B" . $cont, $producto['Producto']['nombre']);
        //     $prueba->setActiveSheetIndex(0)->setCellValue("C" . $cont, $producto['Marca']['nombre']);
        //     $prueba->setActiveSheetIndex(0)->setCellValue("D" . $cont, $producto['Colore']['nombre']);
        //     $nume2 = 68;

        //     $prueba->setActiveSheetIndex(0)->setCellValue(chr($nume2 + 1) . $cont, $total_pro);
        // }
