...
class AmbientesController extends AppController {
	...
	public function reg_ord_pisos() {
		if (!empty($this->request->data)) {
			/* debug($this->request->data);
              exit; */
			foreach ($this->request->data['pisos'] as $key => $pro) {
				$this->Piso->id = $key;
				$datos_p['orden'] = $pro;
				$this->Piso->save($datos_p);
			}
		}
		exit;
	}
	...
	public function ambiente($idPiso = null, $idAmbiente = null, $sw = 0) {
		$this->layout = 'ajax';
	...
	public function guarda_ambiente() {
		if (!empty($this->request->data)) {
			...
			if (empty($valida)) {
				if (!empty($this->request->data['Ambiente']['inquilino_id'])) {
					$this->request->data['Ambiente']['representante_id'] = $this->request->data['Ambiente']['inquilino_id'];
				} elseif (!empty($this->request->data['Ambiente']['propietario_id'])) {
					$this->request->data['Ambiente']['representante_id'] = $this->request->data['Ambiente']['propietario_id'];
				}
	...
	public function inquilino($idPiso = NULL) {
		$this->layout = 'ajax';
		$this->set(compact('idPiso'));
	}
	...
	public function guarda_propietario() {
		...
		if (empty($valida)) {
			...
			//Ingresa los datos a la sesion para salvarla
	...
	public function guarda_inqui() {
		...
		$this->request->data['User']['role'] = 'Inquilino';
		$this->request->data['User']['edificio_id'] = $this->Session->read('Auth.User.edificio_id');
		$array['mensaje'] = '';
		$valida = $this->validar('User');
		if (empty($valida)) {
			...
			$this->request->data['Ambiente']['inquilino_id'] = $idUsuario;
			$this->Session->write('fambiente', $this->request->data);
			//termina la salvada
		} else {
			$array['mensaje'] = $valida;
	...
	public function registra_pagos_mark() {
		...
		foreach ($this->request->data['Dato']['gestion'] as $datos) {
			foreach ($datos['pagos'] as $dat) {
				if ($dat['marca'] == '1') {
					...
					$dato_p['saldo_tmp'] = 0.00;
					$this->Pago->save($dato_p);
				}
			}
		}
		$this->redirect(array('action' => 'listadopago', $idRecibo, $this->request->data['Ambiente']['id']));
	...
	public function cancelar_pago($idRecibo = NULL) {
		...
		$this->redirect(array('controller' => 'Edificios', 'action' => 'ambientes'));
	}
	...
	public function registra_nombre() {
		...
		$array['msgerror'] = '';
		if (empty($valida)) {
			$this->Piso->id = $this->request->data['id'];
			// debug($this->request->data);exit;
			$this->Piso->save($this->request->data);
			$array['estado'] = true;
		} else {
			$array['msgerror'] = $valida;
			$array['estado'] = false;
		}
		$array['nombre_amb'] = $this->request->data['nombre'];
	...
	public function xcobrar($idAmbiente = null) {
		...
		$ambiente = $this->Ambiente->find('first', array(
			...
			'fields' => array('Ambiente.nombre', 'Piso.nombre', 'Ambiente.id', 'Ambiente.user_id', 'Ambiente.fecha_ocupacion', 'Ambiente.saldo', 'Representante.nombre'),
		));
		...
		$edificio = $this->Edificio->findByid($this->Session->read('Auth.User.edificio_id'), null, null, -1);
		$this->set(compact('ambiente', 'recibos', 'conceptos', 'gpagos', 'conceptos_mon', 'idAmbiente', 'deuda_tot_man', 'deuda_tot_alq', 'edificio'));
	}
