...
class UsersController extends AppController {
	...
	public function index() {
		$usuarios = $this->User->find('all', array(
			'conditions' => array('ISNULL(User.deleted)', 'User.role' => array('Super Administrador', 'Administrador')),
		));
	...
	public function lista_usuarios() {
		$usuarios = $this->User->find('all', array(
			'conditions' => array('ISNULL(User.deleted)', 'User.role' => 'Administrador', 'User.edificio_id' => $this->Session->read('Auth.User.edificio_id')),
		));
		$this->set(compact('usuarios'));
	}
	...
	public function eliminar($idUsuario = null) {
		...
		$this->User->id = $idUsuario;
		$d_usuario['deleted'] = date("Y-m-d H:i:s");
		if ($this->User->save($d_usuario)) {
